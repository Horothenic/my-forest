...
namespace MyForest
{
    public class CameraIntroController : MonoBehaviour
    {
        ...
        private void FirstIntro()
        {
            ...
            sequence.AppendCallback(_cameraGesturesControlSource.BlockInput);
            sequence.AppendInterval(0.3f);
            sequence.AppendCallback(_cameraIntroSource.IntroStarted);
            sequence.AppendCallback(() => _cameraGesturesDataSource.SetZoom(40));
            sequence.Append(_blocker.DOFade(0, 0.45f));
            sequence.AppendCallback(() => _container.TurnOff());
            sequence.AppendInterval(0.3f);
            sequence.Append(DOTween.To(() => _cameraGesturesDataSource.CurrentZoom, zoom => _cameraGesturesDataSource.SetZoom(zoom), 10, 2.4f).SetEase(Ease.OutQuart));
